extends ScreenBase
## Run-vyn (US-1.2): rummen, djupet, buren Essens och partyts skick.
## Härifrån går man vidare nedåt – eller överger runnen.
...
func build() -> void:
	...
	for hero_combat in run.heroes_combat:
		total_hp += int(hero_combat["hp"])
		total_max += int(hero_combat["max_hp"])
	layout.add_child(
		UIKit.body(
			(
				"Party %d/%d standing   ·   HP %d/%d"
				% [run.living_heroes_count(), run.heroes_combat.size(), total_hp, total_max]
			),
			18
		)
	)
	...
	for room in run.rooms:
		...
		var pile_mark := (
			"  + your Essence!" if room.get("has_pile", false) and party.has_death_pile() else ""
		)
		...
		if room.get("has_pile", false) and party.has_death_pile():
	...
	if run.is_run_complete():
		...
		# Frivillig sorti: räknas som en wipe (Essensen lämnas på djupet),
...
func _chest_offer(item: Dictionary) -> Control:
	...
	var hero_index: int = Game.run.best_hero_for_item(Game.party, item)
	var hero: Hero = Game.party.heroes[hero_index]
	var current: Dictionary = hero.equipment.get(item["slot"], {})
	if not current.is_empty():
		box.add_child(
			UIKit.body("%s's current: %s" % [hero.hero_name, Items.describe(current)], 16)
		)
	var buttons := HBoxContainer.new()
	buttons.add_theme_constant_override("separation", 10)
	var equip_button := UIKit.big_button("Equip on %s" % hero.hero_name, 70)
	equip_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	equip_button.pressed.connect(
		func():
			Game.run.equip_item(Game.party, item, hero_index)
			Game.save_game()
			main.show_run()
	)
