...
func build() -> void:
	...
	var abandoned: bool = summary.get("abandoned", false)
	...
	layout.add_child(UIKit.title("RUN ABANDONED" if abandoned else "YOU FELL", 44))
	layout.add_child(
		UIKit.body("You fled the depths." if abandoned else "The depths claimed their due.", 18)
	)
	...
	var dropped := int(summary.get("lost_essence", 0)) > 0 or int(summary.get("lost_items", 0)) > 0
	if dropped:
		...
		box.add_child(UIKit.body("You carried nothing of value – nothing was lost.", 18))
